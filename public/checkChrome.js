...
(function() {
    if (location.href.indexOf('browserError') === -1) {
        function getVersionNo(info){
            var version = info.version;
            var versionNo = version.split(".")[0];
            return Number(versionNo)
        }
        var info = new Browser();
        var result = false;
        var allowedBrowser = [
            { browserVersion: "Chrome", allowedVersionNo: 65 },
            { browserVersion: "Firefox", allowedVersionNo: 64 },
            { browserVersion: "Safari", allowedVersionNo: 10 },
            { browserVersion: "Edge", allowedVersionNo: 16 },
        ];
        var browser = info.browser;
        var versionNo = getVersionNo(info);
        for (var i = 0; i < allowedBrowser.length; i += 1){
            if( allowedBrowser[i].browserVersion === browser && allowedBrowser[i].allowedVersionNo <= versionNo) {
               result = true;
            }
        }
        if (info.engine === 'Blink' && Number(navigator.userAgent.replace(/^.*Chrome\/([\d]+).*$/, '$1')) >= 65) {
            result = true;
        }
        if (!result) {
            location.href = "/browserError.html";
        }
    }
